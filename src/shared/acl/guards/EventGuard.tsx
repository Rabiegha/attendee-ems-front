...
export const EventGuard: React.FC<EventGuardProps> = ({
  ...
  if (user?.isSuperAdmin) {
    console.log(
      `Super admin access granted for user ${user?.email} to event ${eventId}`
    )
    return <>{children}</>
  }
  ...
  if (user?.roles?.includes('ADMIN') || user?.roles?.includes('MANAGER')) {
    console.log(
      `Admin/Manager access granted for user ${user?.email} to event ${eventId}`
    )
    return <>{children}</>
  }
  ...
  if (userRole === 'PARTNER' || userRole === 'HOSTESS') {
    ...
    console.log(
      `Assigned role (${userRole}) access granted for user ${user?.email} to event ${eventId} - backend filtering handles assignment`
    )
    return <>{children}</>
  }
  ...
  if (user?.eventIds?.includes(eventId)) {
    console.log(
      `Event access granted for user ${user?.email} to event ${eventId} (in authorized list)`
    )
    return <>{children}</>
  }
  ...
  if (!canAccess) {
    console.log(
      `Access denied for user ${user?.email} to event ${eventId} with action ${action}`
    )
    return <Navigate to={fallbackPath} replace />
  }

  console.log(
    `CASL access granted for user ${user?.email} to event ${eventId} with action ${action}`
  )
  return <>{children}</>
}
