...
const generatePublicToken = () =>
  `evt_pub_${Math.random().toString(36).substr(2, 24)}`
...
export const eventsHandlers = [
  ...
  http.post(`${API_BASE}/events`, async ({ request }) => {
    ...
    const orgId =
      userRole === 'SUPER_ADMIN' && body.org_id ? body.org_id : userOrgId
    ...
    const newEvent: Event = {
      ...
      settings: {
        ...
        registration_auto_approve:
          body.settings?.registration_auto_approve || false,
        allow_checkin_out: body.settings?.allow_checkin_out !== false,
        auto_transition_to_active:
          body.settings?.auto_transition_to_active !== false,
        auto_transition_to_completed:
          body.settings?.auto_transition_to_completed !== false,
  ...
  http.put(`${API_BASE}/events/:id`, async ({ params, request }) => {
    ...
    const updatedEvent: Event = {
      ...
      address_formatted:
        body.location?.address_formatted || event.address_formatted,
