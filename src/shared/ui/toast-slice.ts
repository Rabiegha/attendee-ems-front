...
export const createToastActions = (dispatch: any) => ({
  success: (title: string, message?: string, options?: Partial<Toast>) => {
    const payload: Omit<Toast, 'id'> = { type: 'success' as ToastType, title, ...options }
    if (message !== undefined) payload.message = message
    return dispatch(addToast(payload))
  },
  
  error: (title: string, message?: string, options?: Partial<Toast>) => {
    const payload: Omit<Toast, 'id'> = { type: 'error' as ToastType, title, ...options }
    if (message !== undefined) payload.message = message
    return dispatch(addToast(payload))
  },
  
  warning: (title: string, message?: string, options?: Partial<Toast>) => {
    const payload: Omit<Toast, 'id'> = { type: 'warning' as ToastType, title, ...options }
    if (message !== undefined) payload.message = message
    return dispatch(addToast(payload))
  },
  
  info: (title: string, message?: string, options?: Partial<Toast>) => {
    const payload: Omit<Toast, 'id'> = { type: 'info' as ToastType, title, ...options }
    if (message !== undefined) payload.message = message
    return dispatch(addToast(payload))
  },
