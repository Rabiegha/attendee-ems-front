...
const filterRegistrations = (
  ...
  if (filters.attendeeTypeId) {
    filtered = filtered.filter(r => r.event_attendee_type_id === filters.attendeeTypeId)
  }
  
  if (filters.attendanceType) {
    filtered = filtered.filter(r => r.attendance_type === filters.attendanceType)
  }
...
export const registrationsHandlers = [
  ...
  http.get(`${API_BASE}/events/:eventId/registrations`, ({ params, request }) => {
    ...
    let registrations = mockRegistrations.filter(r => r.event_id === eventId)
    ...
    const registrationsWithAttendees = registrations.map(reg => {
      ...
      if (userRole === 'HOSTESS') {
        return {
          ...
          confirmation_number: `CONF-${eventId.substring(0, 8)}-${reg.id.substring(0, 8)}`.toUpperCase(),
    ...
    const summary = {
      ...
      approved: mockRegistrations.filter(r => r.event_id === eventId && r.status === 'approved').length,
      awaiting: mockRegistrations.filter(r => r.event_id === eventId && r.status === 'awaiting').length,
      refused: mockRegistrations.filter(r => r.event_id === eventId && r.status === 'refused').length,
      cancelled: mockRegistrations.filter(r => r.event_id === eventId && r.status === 'cancelled').length,
      checked_in: 0 // Mock
    }
  ...
  http.put(`${API_BASE}/registrations/:id/status`, async ({ params, request }) => {
    ...
    const attendee = mockAttendees.find(a => a.id === registration.attendee_id)
    if (attendee) {
  ...
  http.post(`${API_BASE}/events/:eventId/registrations/bulk-import`, async ({ params, request }) => {
    const { eventId } = params as { eventId: string }
    ...
    for (let i = 0; i < mockExcelRows.length; i++) {
      ...
      try {
        ...
        Object.keys(row).forEach(key => {
          if (!['email', 'first_name', 'last_name', 'phone', 'company', 'job_title', 'country', 'attendance_type'].includes(key)) {
