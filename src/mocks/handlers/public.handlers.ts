...
import type { Event, Registration, PublicRegisterDTO } from '@/features/events/types'
...
export const publicHandlers = [
  ...
  http.get(`${API_BASE}/public/events/:publicToken`, ({ params }) => {
    ...
    const registrationsForEvent = mockRegistrations.filter(
      r => r.event_id === event.id && ['approved', 'awaiting'].includes(r.status)
    )
    const registeredCount = registrationsForEvent.length
    const remainingSpots = event.capacity ? event.capacity - registeredCount : null
  ...
  // POST /api/public/events/:publicToken/register
  http.post(`${API_BASE}/public/events/:publicToken/register`, async ({ params, request }) => {
    const { publicToken } = params as { publicToken: string }
    ...
    if (!body.email || !body.first_name || !body.last_name) {
      return HttpResponse.json(
        { error: 'Missing required fields: email, first_name, last_name', code: 'VALIDATION_ERROR' },
        { status: 400 }
      )
    }
    ...
    if (existingReg) {
      ...
      if (existingReg.status === 'refused') {
        return HttpResponse.json(
          { 
            error: 'Your registration was previously declined. Please contact the organizer.',
